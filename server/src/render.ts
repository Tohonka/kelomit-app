...
// The report sheet. On screen it sits on the app's dark chrome; on paper it is
// a plain black-on-white A4 document, because that is what a work-hours report
// has to be when it reaches an accountant.
const REPORT_CSS = `
...
.sheet-head h1 { font-size: 1.3rem; }
.sheet-head p { margin: 0.1rem 0 0; }
...
.sheet-table th.right { text-align: right; }
.sheet-table td { padding: 0.45rem 0.5rem; vertical-align: top; }
/* Rule ABOVE each date row, not below it — a row and its "work periods" line
   are one group, and a bottom border would visually attach the detail to the
   following day. */
.sheet-table tbody tr:not(.detail) td { border-top: 1px solid var(--border); }
.sheet-table tbody tr:first-child td { border-top: 0; }
.sheet-table tr.detail td { padding-top: 0; border: 0; }
.sheet-table tr.detail ul { margin: 0.2rem 0 0; padding-left: 1.1rem; }
.sheet-table tr.detail li { font-size: 0.9rem; color: var(--text-secondary); }
...
.stats h2 { font-size: 1.05rem; margin: 0 0 0.75rem; }
.stats h3 {
  font-size: 0.72rem;
  text-transform: uppercase;
  letter-spacing: 0.09em;
...
.stats td { padding: 0.3rem 0.5rem; border-bottom: 1px solid var(--border); }
.print-hint { margin-top: 1rem; font-size: 0.85rem; color: var(--text-muted); }
...
@media print {
  ...
  /* A row and its detail line must not be split across a page. */
  .sheet-table tr { break-inside: avoid; }
  .sheet-table thead { display: table-header-group; }
  .sheet-table tfoot { display: table-footer-group; }
  .stats { break-before: page; }
  h1, h2, h3 { break-after: avoid; }
}
`;

/** Report pages carry the print sheet as well as the app chrome. */

...
describe('occurrences', () => {
  it('once: inside the window only', () => {
    ...
    // Yesterday morning is still inside the 1-day lookback; two days ago is not.
    expect(occurrences({kind: 'once', at: iso(2026, 9, 21, 13)}, NOW)).toHaveLength(1);
    expect(occurrences({kind: 'once', at: iso(2026, 9, 20, 9)}, NOW)).toEqual([]);
  });
...
describe('fireTimes', () => {
  ...
  it('day before / on day / hours before', () => {

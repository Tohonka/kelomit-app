...
import {createCategory, createHabit, updateCategory, updateHabit} from '../db/habits';
import {HABIT_ICONS} from '../components/habits/habitIcons';
import Button from '../components/ui/Button';
import {useTheme, typography, spacing, radius} from '../theme';
import type {Colors} from '../theme';
import type {RootStackScreenProps} from '../navigation/navigationTypes';
import type {HabitGoalKind} from '../types';
...
export default function HabitEditModal({route, navigation}: RootStackScreenProps<'HabitEditModal'>) {
  ...
  const {categories, habits, load} = useHabitStore();
  ...
  const save = async () => {
    ...
    if (mode === 'category') {
      ...
      if (existingHabit) {
        await updateHabit(existingHabit.id, fields);
      } else {
        await createHabit({...fields, category_id: categoryId!});
      }
    }
    await load();
    navigation.goBack();

const en = {
  common: {
    add: 'Add',
    archive: 'Archive',
    cancel: 'Cancel',
    clear: 'Clear',
    done: 'Done',
    close: 'Close',
    yes: 'Yes',
    no: 'No',
    data: 'Data',
    delete: 'Delete',
    discard: 'Discard',
    edit: 'Edit',
    error: 'Error',
    from: 'From',
    gallery: 'Gallery',
    language: 'Language',
    merge: 'Merge',
    none: 'None',
    off: 'Off',
    ok: 'OK',
    on: 'On',
    projects: 'Projects',
    rename: 'Rename',
    replace: 'Replace',
    restore: 'Restore',
    save: 'Save',
    settings: 'Settings',
    stop: 'Stop',
    tags: 'Tags',
    to: 'To',
    update: 'Update',
    version: 'Version',
  },
  manager: {
    tagsProjectsTitle: 'Tags & projects',
    tagsProjectsSubtitle: 'Rename, combine and clean up',
    manageProjects: 'Manage projects',
    manageTags: 'Manage tags',
    newTag: 'New tag',
    tagName: 'Tag name',
    noTags: 'No tags yet.',
    mergeInto: 'Merge into…',
    mergeBannerTag: 'Merge #{{name}} into… tap another tag, or cancel.',
    mergeBannerProject: 'Merge "{{name}}" into… tap another project, or cancel.',
    mergeTitle: 'Combine',
    mergeTagMessage: 'Merge #{{from}} into #{{to}}? Notes tagged #{{from}} will become #{{to}}.',
    mergeProjectMessage: 'Merge "{{from}}" into "{{to}}"? Notes in "{{from}}" move to "{{to}}".',
    deleteTagTitle: 'Delete tag',
    deleteTagMessage: 'Delete #{{name}}? It will be removed from all notes.',
    deleteProjectTitle: 'Delete project',
    deleteProjectMessage: 'Delete "{{name}}"? Notes keep their content but lose this project.',
    nameTakenTag: 'A tag #{{name}} already exists. Use Merge to combine them.',
    nameTakenProject: 'A project "{{name}}" already exists. Use Merge to combine them.',
  },
  activity: {
    work: 'Work',
    personal_work: 'Personal (work)',
    personal: 'Personal',
  },
  projectType: {
    work: 'Work',
    personal: 'Personal',
    other: 'Other',
  },
  entryType: {
    note: 'Note',
    photo: 'Photo',
    video: 'Video',
    voice: 'Voice',
  },
  quickEntryType: {
    note: 'Quick note',
    photo: 'Quick photo',
    video: 'Quick video',
    voice: 'Quick voice note',
  },
  dates: {
    today: 'Today',
    yesterday: 'Yesterday',
    weekShort: 'W',
    weekNumber: 'Week {{week}}',
    previousDay: 'Previous day',
    nextDay: 'Next day',
  },
  navigation: {
    addEntry: 'Add Entry',
    calendar: 'Calendar',
    data: 'Balance',
    editEntry: 'Edit Entry',
    entry: 'Entry',
    gallery: 'Gallery',
    home: 'Today',
    insights: 'Insights',
    interface: 'Interface',
    quickAdd: 'Quick add',
    search: 'Search',
    reorderHint: 'Drag to reorder',
    soon: 'Soon',
    tracking: 'Tracking',
  },
  dayMap: {
    title: 'Route',
    empty: 'Nothing tracked this day.',
    distance: 'Distance',
    duration: 'Duration',
  },
  map: {
    todaysMap: "Today's map",
    fullScreen: 'Full screen',
    locations: 'Locations',
    places: 'Places',
    trips: 'Trips',
    tripDetails: 'Trip details',
    averageSpeed: 'Average speed',
    maximumSpeed: 'Maximum speed',
    averagePace: 'Average pace',
    notMoving: 'Not moving',
    modes: 'Travel modes',
    modeVehicle: 'Driving',
    modeFoot: 'Walking',
    modeCycle: 'Cycling',
    modeStill: 'Paused',
    modeUnknown: 'Unknown',
    viaHeader: 'During the trip',
    viaPause: 'Paused {{duration}} at {{name}} ({{time}})',
    viaPassthrough: 'Passed {{name}} ({{time}})',
    viaUnnamed: 'unnamed stop',
    noVisits: 'No stops tracked yet today.',
    unknownPlace: 'Unknown place',
    savedPlace: 'Saved place',
    reusablePlace: 'Reusable place',
    googlePlace: 'Google',
    distanceAway: '{{distance}} away',
    choosePlace: 'Choose place',
    nameThisPlace: 'Name this place…',
    placeName: 'Place name',
    save: 'Save',
    cancel: 'Cancel',
    placesLoading: 'Loading places…',
    placesError: 'Could not load places.',
    placeSaveError: 'Could not save place name.',
    unknown: 'Unknown',
    dayStart: 'Day start',
    dayEnd: 'Day end',
    addTripToNote: 'Add trip to note',
    saveTripsAsNote_one: 'Save trip as note',
    saveTripsAsNote_other: 'Save {{count}} trips as note',
  },
  gallery: {
    groupDay: 'Day',
    groupWeek: 'Week',
    groupMonth: 'Month',
    viewGrid: 'Grid',
    viewMap: 'Map',
    empty: 'No photos or videos yet.\nThey show up here as you capture them.',
    noLocations: 'No located photos yet.\nGeotagged photos appear on the map.',
    fileMissing: 'This file is no longer available.',
    zoomHint: 'Pinch to zoom · double-tap to reset',
    notesHere: 'Notes here',
  },
  day: {
    detailsTitle: 'Day details',
    projectSplit: 'By project',
  },
  dayEnd: {
    channelName: 'Workday end',
    notificationTitle: 'End of workday?',
    question: 'Did your workday end at {{time}}?',
  },
  dayNote: {
    title: 'Day note',
    add: '+ Note for this day',
    placeholder: 'Sick day, office closed early, …',
  },
  app: {
    dbErrorTitle: 'Failed to open database:',
  },
  settings: {
    app: 'App',
    interfaceTitle: 'Interface',
    interfaceSubtitle: 'Theme, week numbers, time entry',
    quickAddTitle: 'Quick add',
    quickAddSubtitle: 'Defaults for long-press add',
    trackingTitle: 'Tracking',
    trackingSubtitle: 'GPS, defaults',
    workDetailsTitle: 'Work details',
    workDetailsSubtitle: 'Usual hours, per-weekday hours',
    defaultHours: 'Default hours',
    perWeekday: 'Per weekday',
    perWeekdayHint: 'Override the default for specific weekdays. Tap a day to cycle: default → custom → day off.',
    weekdayMode_default: 'Default',
    weekdayMode_custom: 'Custom',
    weekdayMode_off: 'Day off',
    dataTitle: 'Data',
    dataSubtitle: 'Projects, export',
    appearance: 'Appearance',
    theme: 'Theme',
    themeAuto: 'Auto',
    themeLight: 'Light',
    themeDark: 'Dark',
    colorTheme: 'Colors',
    colorThemeDefault: 'Default',
    colorThemeHornet: 'Hornet',
    languageEnglish: 'English',
    languageFinnish: 'Suomi',
    weekNumbers: 'Week numbers',
    personalHours: 'Show personal hours',
    personalHoursDescription: 'Adds a second personal-time line under the work total',
    subnotesExpanded: 'Expand subnotes by default',
    subnotesExpandedDescription: 'Day lists open with subnotes shown under their note',
    navVisibility: 'Bottom navigation',
    navVisibilityDescription: 'Where the tab bar is shown',
    navVisibilityAlways: 'Always',
    navVisibilityHomeOnly: 'Home only',
    workingHours: 'Working hours',
    usualStart: 'Usual start',
    usualEnd: 'Usual end',
    prefillNewDays: 'Prefill new days',
    prefillNewDaysDescription: 'Seed each new day with your usual hours (still editable)',
    payPeriod: 'Pay period',
    payPeriodStart: 'Pay period start day',
    payPeriodHint: '{{start}} → {{end}}',
    monthEnd: 'month end',
    weeklyTarget: 'Weekly hours target',
    weeklyTargetHint: 'The Balance ring measures the week against this',
    timeNotSet: 'Not set',
    timeEntry: 'Time entry',
    timeSelector: 'Time selector',
    timeSelectorClock: 'Clock',
    timeSelectorKeyboard: 'Type',
    timeSelectorClockDescription: 'Analog clock dialog',
    timeSelectorKeyboardDescription: 'Type the time directly',
    quickAddDefaults: 'Quick add defaults',
    tag: 'Tag',
    project: 'Project',
    noTag: 'No tag',
    location: 'Location',
    gpsTracking: 'GPS tracking',
    backgroundTracking: 'Track in background',
    backgroundTrackingDescription: 'Keep logging location when the app is closed, so arriving and leaving work are detected at the right time. Shows a quiet ongoing notification.',
    defaults: 'Defaults',
    defaultActivity: 'Default activity',
    exportCsv: 'Export CSV',
    exporting: 'Exporting…',
    shareCsv: 'Share CSV',
    exportFailed: 'Export failed',
    exportDone: 'CSV saved',
    exportNoEntries: 'No entries found in the selected date range.',
    exportDialogTitle: 'Export data',
    exportShareMessage: 'Kelomit export {{startDate}} → {{endDate}} ({{count}} entries)',
    backupSection: 'Backup & restore',
    backup: 'Back up everything',
    backupSubtitle: 'Save a full backup (data + photos) you can share or store',
    restore: 'Restore from backup',
    restoreSubtitle: 'Replace all current data with a backup file',
    backupFileName: 'Kelomit backup',
    backupShareMessage: 'Kelomit full backup (database + media)',
    backupDialogTitle: 'Save backup',
    backupSavedTitle: 'Backup saved',
    backupSavedMessage: 'Your full backup has been saved.',
    backupFailed: 'Backup failed',
    restoreConfirmTitle: 'Restore from backup?',
    restoreConfirmMessage: 'This replaces ALL current data and photos with the chosen backup. A safety copy of your current data is saved first. Continue?',
    restoreConfirmCta: 'Choose backup',
    restoreInvalid: "That file isn't a valid Kelomit backup.",
    restoreFailed: 'Restore failed',
    restoreDoneTitle: 'Restore complete',
    restoreDoneMessage: 'Your data has been restored. Please fully close and reopen the app to finish.',
    syncSection: 'Remote sync',
    syncServerUrl: 'Server URL',
    syncToken: 'Access token',
    syncNow: 'Sync now',
    syncing: 'Syncing…',
    syncNeverRun: 'Never synced',
    offNotice: 'Food barcode lookups are sent to Open Food Facts (the barcode only). Results are stored on this device.',
    foodData: 'Food data',
    foodDataValue: 'Fineli — Finnish Institute for Health and Welfare (THL), CC BY 4.0 · Open Food Facts, ODbL',
    syncLastAt: 'Last sync: {{when}}',
    syncNotConfigured: 'Set a server URL and token first',
    syncFailed: 'Last sync failed: {{error}}',
    syncDone: 'Sync complete',
    syncSave: 'Save',
    companionSection: 'Desktop companion',
    companionUrl: 'Companion address (http://…)',
    companionToken: 'Pairing token',
    companionScan: 'Scan pairing QR',
    companionScanBad: 'That is not a Kelomit pairing code',
    companionPushNow: 'Push now',
    companionPushing: 'Pushing…',
    companionNeverPushed: 'Never pushed',
    companionLastPush: 'Last push: {{when}}',
    companionFailed: 'Last push failed: {{error}}',
    companionNotConfigured: 'Pair with the desktop app first',
    companionDone: 'Push complete',
    pausedUntil: 'Tracking paused until {{time}}',
    pausedIndefinitely: 'Tracking paused',
    resume: 'Resume',
  },
  reporting: {
    title: 'Reporting',
    subtitle: 'Export work hours as a PDF',
    identity: 'Report details',
    personName: 'Your name',
    personNamePlaceholder: 'Matti Meikäläinen',
    companyName: 'Company name',
    companyNamePlaceholder: 'Kelo Design Oy',
    dateRange: 'Date range',
    startDateAccessibility: 'Select start date',
    endDateAccessibility: 'Select end date',
    language: 'Report language',
    languageFi: 'Finnish',
    languageEn: 'English',
    languageFiAccessibility: 'Report language Finnish',
    languageEnAccessibility: 'Report language English',
    type: 'Report content',
    typeHours: 'Daily hours',
    typeHeadlines: 'Hours and headlines',
    typeStatistics: 'Hours and statistics',
    typeHoursAccessibility: 'Report type Daily hours',
    typeHeadlinesAccessibility: 'Report type Hours and headlines',
    typeStatisticsAccessibility: 'Report type Hours and statistics',
    templateWorkHours: 'Work hours',
    export: 'Export PDF',
    exporting: 'Exporting…',
    errorPersonRequired: 'Enter your name.',
    errorCompanyRequired: 'Enter the company name.',
    errorInvalidRange: 'The start date must be on or before the end date.',
    errorEmpty: 'No worked hours were found in the selected date range.',
    errorReadFailed: 'The report data could not be read.',
    errorRenderFailed: 'The report PDF could not be created.',
    errorSaveFailed: 'The report could not be saved.',
    errorLoadIdentity: 'The saved report details could not be loaded.',
    errorSaveIdentity: 'The report details could not be saved.',
  },
  calendar: {
    month: 'Month',
    week: 'Week',
    range: 'Range',
    totalWorkThisPeriod: 'Total work this period',
    weekdaysShort: {
      mon: 'Mon',
      tue: 'Tue',
      wed: 'Wed',
      thu: 'Thu',
      fri: 'Fri',
      sat: 'Sat',
      sun: 'Sun',
    },
  },
  leave: {
    paidDayOff: 'Day off (paid)',
    unpaidDayOff: 'Day off (unpaid)',
    vacation: 'Vacation',
    sick: 'Sick',
    paidShort: 'Paid',
    unpaidShort: 'Unpaid',
    vacationShort: 'Vac',
    sickShort: 'Sick',
    startDate: 'Start date',
    endDate: 'End date',
    overlap: 'This leave overlaps an incompatible leave period.',
    invalidRange: 'The start date must be before the end date.',
    loadFailed: 'Could not load leave.',
    saveFailed: 'Could not save leave.',
    deleteTitle: 'Delete leave?',
    deleteMessage: 'This removes the whole linked leave period.',
  },
  entries: {
    noteTab: 'Note',
    leaveTab: 'Leave',
    overtime: 'Overtime',
    noEntries: 'No entries yet. Tap + to add one.',
    sortTimeDesc: 'Newest first',
    sortTimeAsc: 'Oldest first',
    sortProject: 'By project',
    sortType: 'By type',
    noProject: 'No project',
    listTitle: 'Entries',
    type: 'Type',
    activity: 'Activity',
    title: 'Title',
    titleOptional: 'Title (optional)',
    attachments: 'Attachments',
    removeAttachmentTitle: 'Remove attachment',
    removeAttachmentMessage: 'Delete this attachment from the note?',
    titlePlaceholder: 'Short title…',
    note: 'Note',
    notePlaceholder: 'Write something…',
    projectOptional: 'Project (optional)',
    tags: 'Tags',
    tagPlaceholder: 'Add a tag…',
    timeTrackingOptional: 'Time tracking (optional)',
    startOptional: 'Start (optional, defaults to now)',
    noTimeTracking: 'None',
    duration: 'Duration',
    durationOptional: 'Duration (optional)',
    fromTo: 'From → To',
    minutes: 'Minutes',
    minuteUnit: 'min',
    photo: 'Photo',
    video: 'Video',
    voiceRecording: 'Voice recording',
    quickTitlePlaceholder: 'What is it?',
    quickDefaults: 'Saved as {{activity}}{{tag}}',
    createProjectAs: 'Create',
    createProjectAsSuffix: 'as',
    searchProjectPlaceholder: 'Search or add a project…',
    videoFileSaved: 'Video file saved',
    created: 'Created',
    location: 'Location',
    deleteEntry: 'Delete entry',
    deleteTitle: 'Delete entry',
    deleteMessage: 'This action cannot be undone.',
    loadingError: 'Error',
  },
  media: {
    camera: 'Camera',
    gallery: 'Gallery',
    errorSavingPhoto: 'Error saving photo',
    errorSavingVideo: 'Error saving video',
    videoCaptured: 'Video captured',
    recordingError: 'Recording error',
    stopRecordingError: 'Stop recording error',
    recordingSaved: 'Recording saved',
    tapToRecord: 'Tap to record',
    play: 'Play',
  },
  transcription: {
    title: 'Transcription',
    subtitle: 'Speech-to-text for voice notes',
    provider: 'Provider: OpenAI Whisper API',
    engineTitle: 'Transcription engine',
    engineOnDevice: 'On-device (Whisper)',
    engineApi: 'OpenAI API',
    languageTitle: 'Transcription language',
    languageHint: 'On-device auto-detect is weak; pick your spoken language.',
    languageAuto: 'Auto',
    languageFi: 'Finnish',
    languageEn: 'English',
    modelTitle: 'On-device model',
    modelNotDownloaded: 'Not downloaded (~140 MB)',
    modelReady: 'Ready',
    modelDownloading: 'Downloading… {{pct}}%',
    download: 'Download',
    delete: 'Delete',
    downloadFailed: 'Download failed. Check your connection and retry.',
    keyLabel: 'OpenAI API key',
    keyPlaceholder: 'sk-...',
    keySet: 'Key saved',
    keyNotSet: 'No key set',
    save: 'Save key',
    clear: 'Clear key',
    saved: 'API key saved',
    cleared: 'API key cleared',
    transcribe: 'Transcribe',
    transcribing: 'Transcribing…',
    retranscribe: 'Re-transcribe',
    transcriptLabel: 'Transcript',
    useAsNote: 'Use as note text',
    cleanupTitle: 'Clean up with AI',
    cleanupLabel: 'Fix wording and write the title',
    cleanupHint:
      'Sends the transcript to OpenAI to fix mis-heard words and derive a short title. Needs the API key above. The recording and the raw transcript are always kept.',
    cleanupNeedsKey: 'Set an OpenAI API key below to use this.',
    errNoKey: 'Set an OpenAI API key first.',
    errModelMissing: 'The on-device model isn\'t downloaded. Open settings to download it.',
    errAuth: 'API key was rejected. Check it in settings.',
    errRate: 'Rate limit or spend cap reached. Try later.',
    errNetwork: 'No connection. Check your network.',
    errOther: 'Transcription failed. Try again.',
    errFileMissing: 'Recording file not found. Try recording again.',
    errNoSpeech: 'No speech detected in this recording.',
    goToSettings: 'Open settings',
  },
  diagnostics: {
    title: 'Diagnostics',
    subtitle: 'On-device log for debugging GPS & background service',
    logTitle: 'Diagnostic log',
    fileSize: 'Log file: {{size}}',
    rowCount: 'Structured events: {{count}}',
    hint: 'Captures service lifecycle, Doze kills, provider health, watch stalls and app state. Share the file to analyse on desktop.',
    share: 'Share log',
    clear: 'Clear',
    clearConfirmTitle: 'Clear diagnostics?',
    clearConfirmMsg: 'Deletes the log file and stored events. This cannot be undone.',
    empty: 'The log is empty.',
    shareFailed: 'Could not share the log.',
  },
  time: {
    clearTime: 'Clear this time?',
    enterTime: 'Enter time',
    formatHint: '24-hour format · 00:00 – 23:59',
    noTimeTracked: 'No time tracked yet',
    start: 'Start',
    end: 'End',
    worked: 'Worked',
    adjAfterHours: 'after hours',
    adjPersonal: 'personal',
  },
  timer: {
    title: 'Quick timer',
    start: 'Start timer',
    stop: 'Stop',
    cancelTimer: 'Discard',
    tracking: 'Tracking…',
    pause: 'Pause',
    resume: 'Resume',
    paused: 'Paused',
    started: 'Started {{time}}',
    noteTitle: 'Timer note {{time}}',
    options: 'Options',
    titlePlaceholder: 'What are you working on?',
    cancelConfirmTitle: 'Discard timer?',
    cancelConfirmMessage: "Stop tracking without saving? This timed session won't be logged.",
  },
  widgets: {
    title: 'Widgets',
    settingsSubtitle: 'Home-screen timer widgets',
    intro: 'Add a Kelomit timer widget to your home screen, then choose what each one tracks.',
    none: 'No widgets placed yet. Long-press your home screen, pick Widgets, and add a Kelomit timer.',
    unavailable: 'Home-screen widgets need the latest app build. Rebuild and reinstall to use them.',
    typeFull: 'Timer widget',
    typeToggle: 'Toggle widget',
    typeHabits: 'Habit widget',
    addHabits: 'Add habit widget',
    addFood: 'Add food widget',
    addNag: 'Add nag widget',
    habitsPick: 'Habits on this widget (up to 5, in order)',
    habitsShowName: 'Show habit names',
    habitsHint: 'A 1×1 widget shows one habit; make it wider for more. Tap a square to tick the habit for today.',
    name: 'Name',
    namePlaceholder: 'e.g. Project Banana',
    tagPlaceholder: 'Tags, comma-separated…',
    saved: 'Saved',
    addFull: 'Add timer widget',
    addToggle: 'Add toggle widget',
    addAddNote: 'Add note widget',
    addTracking: 'Add tracking widget',
    pinUnsupportedTitle: 'Not supported',
    pinUnsupported: 'Your launcher doesn’t support adding widgets from the app. Long-press the home screen and pick Widgets instead.',
    voiceAutoTitle: 'Auto-title voice notes',
    voiceAutoTitleHint: 'Widget voice notes are transcribed after saving and titled from the first spoken words.',
    voiceMode: 'Voice notes from widget',
    voiceModeConfirm: 'Confirm save',
    voiceModeAuto: 'Fully automatic',
    voiceModeConfirmHint: 'After recording, review the note and tap Save yourself.',
    voiceModeAutoHint: 'The note saves itself the moment you stop recording, then gets transcribed and titled.',
    addSection: 'Add a widget',
    placedSection: 'Placed widgets',
    editTitle: 'Widget',
  },
  projects: {
    newProject: 'New project',
    newProjectButton: '+ New project',
    projectName: 'Project name',
    nameRequired: 'Name required',
    archiveTitle: 'Archive project',
    archiveMessage: 'Archive "{{name}}"?',
    showArchived: 'Show archived',
    hideArchived: 'Hide archived',
  },
  search: {
    placeholder: 'Search notes, tags, projects…',
    noMatches: 'No matches for “{{query}}”.',
    prompt: 'Search across your notes, tags and projects.',
    results_one: '{{count}} result',
    results_other: '{{count}} results',
  },
  balance: {
    work: 'Work',
    movement: 'Movement',
    walking: 'Walking',
    cycling: 'Cycling',
    driving: 'Driving',
    still: 'Stationary',
    energyEstimate: '~{{kcal}} kcal from moving (estimate)',
    energyNeedsWeight: 'Add your weight in Settings → Health for an energy estimate',
    walkingMinutes: 'Walking minutes',
    habits: 'Habits',
    habitDays: '{{done}} / {{total}} days',
    food: 'Food',
    foodTotal: '{{kcal}} kcal · {{n}} entries',
    foodAvg: '{{kcal}} kcal / day on {{days}} days',
    foodNoKcal: '{{n}} without kcal',
    dailyKcal: 'Daily kcal',
    health: 'Health',
    steps: 'Steps',
    perDay: '{{n}} / day',
    sleep: 'Sleep',
    sleepAvg: '{{h}} h {{m}} min / night',
    weight: 'Weight',
    weightTrend: '{{from}} → {{to}} kg',
    restingHr: 'Resting heart rate',
    bpm: '{{n}} bpm',
    dailySteps: 'Daily steps',
    empty: 'Nothing tracked in this period yet.',
  },
  insights: {
    thisWeek: 'This week',
    thisMonth: 'This month',
    last30: 'Last 30 days',
    empty: 'No tracked time in this period yet.',
    scopeAll: 'All',
    scopeWork: 'Work',
    scopePersonal: 'Personal',
    totalTracked: 'Total tracked',
    outOfWorkHours: 'of {{worked}} worked',
    byActivity: 'By activity',
    byProject: 'By project',
    byTag: 'By tag',
    noProject: 'No project',
    trackedThisWeek: 'Tracked this week',
    ofTarget: 'of target',
    ofTargetHours: 'of {{target}} target',
    dailyHours: 'Daily hours',
  },
  todo: {
    schedule: 'Schedule as to-do',
    scheduleHint: "Plan this for a future day. It won't count as worked hours until you confirm it.",
    scheduledFor: 'Scheduled for',
    badge: 'To-do',
    markDone: 'Mark done',
    done: 'Done',
    reopen: 'Reopen',
    comingUp: 'Coming up',
    availableOn: 'You can confirm this on {{date}}',
    remindMe: 'Remind me',
    reminderTime: 'Reminder',
    channelName: 'To-do reminders',
    notificationTitle: 'To-do reminder',
    notificationBody: 'You have a planned item.',
  },
  location: {
    title: 'Location',
    settingsSubtitle: 'Work & home places',
    savedLocations: 'Saved places',
    noLocations: 'No places saved yet.',
    addCurrent: 'Save current position',
    atWorkNow: "I'm at work now",
    atHomeNow: "I'm at home now",
    saveHere: 'Save this place',
    namePlaceholder: 'Place name (optional)',
    work: 'Work',
    home: 'Home',
    detected: 'Detected: {{place}}',
    placeOther: 'Other',
    placeUnknown: 'Unknown',
    radius: '{{m}} m radius',
    decreaseRadius: 'Decrease radius',
    increaseRadius: 'Increase radius',
    editRadius: 'Edit radius',
    radiusFloorHint: "It's not recommended to go below 20 meters.",
    noPosition: 'Could not get your location. Make sure location is on and try again.',
    autoStampHint: 'Arriving at a Work place auto-fills the day start time, and leaving fills the end time — only when you haven\'t set them yourself.',
    deleteTitle: 'Remove place',
    deleteMessage: 'Remove "{{name}}"?',
    whereAmI: 'Where am I now',
    whereAmIHint: 'Force a location check and see which saved place you\'re in.',
    checkPosition: 'Check my position',
    checking: 'Checking…',
    accuracy: '±{{m}} m',
    noPlaceMatch: 'Not inside any saved place',
  },
  places: {
    title: 'Place names',
    settingsSubtitle: 'Rename places, tune geofence sizes',
    savedSection: 'Saved places',
    reusableSection: 'Named stops',
    rename: 'Rename',
    renameHint: 'Renaming changes future trips; past days keep the name they were saved with.',
    empty: 'Nothing here yet.',
    deleteTitle: 'Delete place',
    deleteMessage: 'Delete "{{name}}"? Past days keep their saved names.',
  },
  permissions: {
    requiredTitle: 'Permission required',
    requiredMessage: 'Please enable this permission in your device Settings to use this feature.',
    backgroundLocationTitle: 'Allow all the time',
    backgroundLocationMessage: 'To track in the background, set Location access to "Allow all the time" for Kelomit in your device Settings.',
  },
  fab: {
    more: 'More',
    less: 'Less',
  },
  subnotes: {
    moveParentStart: 'Parent starts at {{current}}. Move it to start at {{next}}?',
    moveParentEnd: 'Parent ends at {{current}}. Move it to end at {{next}}?',
    tooLong: "A subnote can't be longer than its parent ({{length}}).",
    title: 'Subnotes',
    add: 'Add subnote',
    subnoteOf: 'Subnote of',
    partOf: 'Part of',
    untitled: 'Untitled note',
    showAll: 'Show subnotes',
    collapse: 'Hide subnotes',
    voiceSaved: 'Voice note saved',
    photoSaved: 'Photo saved',
    savedToTimer: 'Saved under the running timer',
  },
  nags: {
    title: 'Nags',
    channelName: 'Nags',
    new: 'New nag',
    edit: 'Edit nag',
    empty: 'Nothing nagging yet. Add one for the things that must not slip.',
    nextDue: 'Next',
    noNext: 'No upcoming time',
    today: 'Today',
    tomorrow: 'Tomorrow',
    overdue: 'Overdue',
    dueBy: 'Needs to be done by {{time}}',
    wasDueAt: 'Was due at {{time}} — still not done',
    snooze: 'Snooze 30 min',
    doneToday: 'Done',
    homeStrip: 'Nags today',
    titleLabel: 'What',
    titlePlaceholder: 'Take the meds, call the bank…',
    noteLabel: 'Note',
    when: 'When',
    kindOnce: 'Once',
    kindDates: 'Dates',
    kindWeekly: 'Weekly',
    addDate: 'Add date',
    weekdays: 'Weekdays',
    time: 'Time',
    nagging: 'Nagging',
    naggingHint: 'Each notification repeats until you mark it done.',
    dayBefore: 'The day before at',
    onDay: 'On the day at',
    hoursBefore: 'Hours before',
    repeat: 'Repeat until done',
    perHour: 'per hour',
    fromBefore: 'from',
    untilAfter: 'h before until',
    afterUnit: 'h after',
    random: 'Random times',
    countdown: 'Show countdown',
    alarmBanner: 'Android is blocking exact alarms, so nags may arrive late.',
    alarmFix: 'Allow',
    delete: 'Delete nag',
    deleteConfirm: 'Delete this nag and its history?',
    workPersonal: 'Counts as',
  },
  habits: {
    title: 'Habits',
    newCategory: 'New group',
    editCategory: 'Edit group',
    newHabit: 'New habit',
    editHabit: 'Edit habit',
    titleField: 'Name',
    titlePlaceholder: 'e.g. Language learning',
    description: 'Description',
    descriptionPlaceholder: 'Optional',
    icon: 'Icon',
    color: 'Colour',
    keywordPlaceholder: 'Project, tag or new keyword…',
    streakGoal: 'Streak goal',
    days: 'days',
    goal: 'Daily goal',
    goalNone: 'Once a day',
    goalMinutes: 'Minutes',
    goalCount: 'Times',
    minutesSuffix: 'min / day',
    timesSuffix: '× / day',
    minutesPerDay: '{{n}} min/day',
    timesPerDay: '{{n}}×/day',
    empty: 'No habits yet. Start with a group, e.g. "Language learning", then add habits to it.',
    noHabits: 'No habits in this group yet.',
    deleteCategoryTitle: 'Delete group',
    deleteCategoryMessage: 'Delete "{{name}}" and all its habits?',
    deleteHabitTitle: 'Delete habit',
    deleteHabitMessage: 'Delete "{{name}}"? Its manual ticks are lost.',
    matchers: 'Counts automatically when a note has…',
    matchHint: 'Any selected project, tag or trigger on a day’s note marks the habit done (time goals add up the notes’ tracked time).',
    lifeSection: 'Or from health & food',
    lifeHint: 'Counts the day when the total reaches the number (food kcal: stays under it). Needs Health Connect / the food log.',
    lifeSteps: 'Steps ≥',
    lifeSleep: 'Sleep minutes ≥',
    lifeFoodEntries: 'Food entries ≥',
    lifeFoodKcal: 'Food kcal ≤',
    streak: '🔥 {{n}} / {{goal}} days',
    streakNoGoal: '🔥 {{n}} days',
    progressMin: '{{done}} / {{goal}} min today',
    progressCount: '{{done}} / {{goal}}× today',
  },
  health: {
    title: 'Health',
    subtitle: 'Health Connect, body profile',
    statusAvailable: 'Health Connect is available',
    statusUpdate: 'Health Connect needs an update',
    statusUnavailable: 'Health Connect is not available on this device',
    connect: 'Read from Health Connect',
    connectDescription: 'Steps, distance, calories, sleep, exercise sessions, weight, height, resting heart rate. Only daily totals are stored.',
    writeFood: 'Write food to Health Connect',
    writeFoodDescription: 'Entries with kcal appear as Nutrition records in Health Connect. Only entries logged after switching this on.',
    permissionsDenied: 'Permissions were not granted.',
    importNow: 'Import now',
    importing: 'Importing…',
    lastImport: 'Last import: {{when}}',
    neverImported: 'Not imported yet',
    imported: '{{n}} days updated',
    today: 'Today',
    steps: '{{n}} steps',
    sleep: '{{h}} h {{m}} min sleep',
    weight: '{{kg}} kg',
    noData: 'No data for today yet',
    openSettings: 'Open Health Connect',
    importHistory: 'Import history…',
    importHistoryHint: 'Health Connect keeps what your apps have written there. The first import took only 90 days.',
    months3: 'Last 3 months',
    months12: 'Last 12 months',
    years5: 'Everything (up to 5 years)',
    updateBody: 'Update weight & height now',
    updateBodyHint: 'From the latest Health Connect readings',
    updated: 'Updated',
    noBodyData: 'No weight or height imported yet',
    bodySection: 'Body profile',
    bodyHint: 'Used for energy estimates when Health Connect has no weight or height.',
    weightKg: 'Weight (kg)',
    heightCm: 'Height (cm)',
    birthYear: 'Birth year',
    sex: 'Sex',
    sexMale: 'Male',
    sexFemale: 'Female',
    sexUnset: '–',
  },
  food: {
    title: 'Food',
    logFood: 'Log food',
    editFood: 'Edit food',
    name: 'What did you eat?',
    namePlaceholder: 'e.g. rye bread and cheese',
    kcal: 'kcal (optional)',
    kcalUnit: 'kcal',
    time: 'Time',
    date: 'Date',
    note: 'Note (optional)',
    photo: 'Photo',
    photoPlaceholder: 'Photo',
    recents: 'Recent',
    emptyDay: 'Nothing logged yet.',
    summary: '{{kcal}} kcal · {{n}} entries',
    withoutKcal: '{{n}} without kcal',
    addAgain: 'Add again',
    duplicate: 'Duplicate & tweak',
    added: 'Added',
    deleteTitle: 'Delete entry',
    deleteMessage: 'Delete "{{name}}"?',
    scan: 'Scan barcode',
    product: 'Product',
    quantity: 'Amount',
    unitG: 'g',
    unitServing: 'serving',
    unitPiece: 'pcs',
    myFoods: 'My foods',
    myFoodsHint: 'Search my foods…',
    myFoodsEmpty: 'No match.',
    eaten: 'Eaten',
    sort: 'Sort',
    sortBy: {time: 'Time', kcal: 'Energy', name: 'Name'},
    perHundred: '{{kcal}} kcal / 100 g',
    pendingBarcode: 'Barcode {{code}} will be remembered with this entry.',
    barcodeUnknown: 'Not in Open Food Facts — fill it in once and it’s remembered.',
    barcodeOffline: 'No connection — fill it in once and it’s remembered.',
    scanFailed: 'The barcode scanner is not available on this device.',
    sourceFineli: 'Fineli',
    sourceOwn: 'Own',
  },
  energy: {
    title: 'Energy',
    bmr: 'BMR',
    used: 'Used (estimate)',
    usedSoFar: 'Used so far',
    eaten: 'Eaten',
    total: 'Total',
    assumed: 'assumed',
    healthConnectSays: 'Health Connect’s own figure',
    note: 'Factorial estimate: basal rate {{bmr}} kcal/day (Mifflin-St Jeor) × an activity ratio for each part of the day. Expect ±15 %.',
    needsProfile: 'Energy use needs your {{fields}} — tap to fill in.',
    field: {weight: 'weight', height: 'height', birthYear: 'birth year', sex: 'sex'},
    bucket: {
      sleep: 'Sleep',
      exercise: 'Exercise',
      walk: 'Walking / running',
      cycle: 'Cycling',
      steps: 'Other steps',
      vehicle: 'In a vehicle',
      work: 'Work',
      rest: 'Everything else',
    },
    workActivity: 'Work is',
    work: {desk: 'Desk', mixed: 'Mixed', physical: 'Physical'},
    workActivityHint: 'How physical a counted work hour is. Only used for the energy estimate.',
    kcalPerDay: '{{kcal}} kcal/day',
    totalAndAvg: '{{total}} kcal · {{avg}}/day',
    eatenOnDays: 'Eaten ({{days}} logged days)',
    estimateNote: 'Finished days only. Estimates, not measurements.',
    dailyUsed: 'Energy used per day',
  },
  profile: {
    me: 'Me',
    namePlaceholder: 'Your name',
    changePhoto: 'Change profile photo',
    addBody: 'Add weight, height and birth year',
    age: '{{n}} y',
    weekHours: 'Work this week / {{target}} h',
    widgets: 'Widgets',
    data: 'Data & backup',
  },
  patterns: {
    title: 'Patterns',
    sentence: 'Days with {{x}} {{cmp}} {{threshold}}: {{y}} averaged {{low}}. Other days: {{high}}.',
    sentenceNext: 'Days with {{x}} {{cmp}} {{threshold}}: the next day’s {{y}} averaged {{low}}. Other days: {{high}}.',
    series: {sleep: 'sleep', steps: 'steps', exercise: 'exercise', kcal: 'eating', work: 'work', habits: 'habits done'},
    under: 'under',
    atMost: 'at most',
    needMore: '{{have}} usable days so far — patterns need about {{need}}.',
    none: 'Nothing stands out yet.',
    note: 'Last {{days}} days, split at the median. Coincidences happen — these are observations, not causes.',
  },
  smallTask: {
    duringWorkday: 'During workday',
    positionHint: 'Position in list (optional)',
    outsideHoursNotice: 'Small tasks are meant for things done during the workday. Saved anyway.',
    groupTitle: 'Tasks',
    badge: 'Small task',
  },
};

export default en;
